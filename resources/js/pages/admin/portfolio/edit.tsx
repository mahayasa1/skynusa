...
import { ArrowLeft, Save, Briefcase, X, Trash2 } from 'lucide-react';
...
export default function PortfolioEdit({ portfolio, services, companies, categories }: PortfolioEditProps) {
    const [imagePreview, setImagePreview] = useState<string | null>(
        portfolio.image ? `/storage/${portfolio.image}` : null
    );
    const [galleryPreviews, setGalleryPreviews] = useState<string[]>(
        portfolio.gallery?.map(img => `/storage/${img}`) || []
    );
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'Edit', href: `/admin/portfolio/${portfolio.id}/edit` },
    ];

    const { data, setData, post, processing, errors } = useForm({
        service_id: portfolio.service_id.toString(),
        companies_id: portfolio.companies_id.toString(),
        categories_id: portfolio.categories_id.toString(),
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-gray-600 mt-1">Perbarui data portfolio {portfolio.title}</p>
                        </div>
                    </div>
                </div>
                            ...
                            <Card className="shadow-md bg-white text-black">
                                <CardContent className="p-4 space-y-2">
                                    ...
                                    >
                                        ...
                                        {processing ? 'Menyimpan...' : 'Update Portfolio'}
                                    </Button>
                                    <Link href="/admin/portfolio">
                                        <Button
                                            type="button"
